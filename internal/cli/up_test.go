...
// preflight's `sudo -n install --version` probe reports "password is required",
// triggering CheckPrivilegePasswordReq.
...
func TestUp_HostInstallRequired_JSON(t *testing.T) {
	stateDir := t.TempDir()
	service := newFakePermittedGitHubService()
	remoteExec := newFakeRemoteExecutor()
	...
	err := cmd.Execute()
	if err == nil {
		t.Fatal("expected host_install_required error")
	}
	if got := ExitCode(err); got != ExitInputRequired {
		t.Fatalf("ExitCode() = %d, want %d", got, ExitInputRequired)
	}
	combined := out.String()
	if !strings.Contains(combined, "host_install_required") {
		t.Fatalf("stdout missing host_install_required:\n%s", combined)
	}
	if !strings.Contains(combined, `"schema_version"`) {
		t.Fatalf("stdout missing schema_version:\n%s", combined)
	}
	if !strings.Contains(combined, "next_actions") {
		t.Fatalf("stdout missing next_actions:\n%s", combined)
	}
}

func TestUp_HostInstallRequired_Human(t *testing.T) {
	stateDir := t.TempDir()
	service := newFakePermittedGitHubService()
	remoteExec := newFakeRemoteExecutor()
	passwordRequiredProbe(remoteExec)
	...
	if !strings.Contains(combined, "host_install_required") && !strings.Contains(combined, "one-time host install") {
		t.Fatalf("expected host install remediation in output:\n%s", combined)
	}
}
