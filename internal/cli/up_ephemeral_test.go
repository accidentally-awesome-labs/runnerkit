...
// ListRunners response uses the ephemeral runner name (with the random
// short-id suffix) returned by the test's runtime. We accept whatever
// labelSet name the up flow generated by recording every ListRunners
// call and matching label prefixes.
...
func (s *fakeEphemeralGitHubService) ListRunners(_ context.Context, repo gh.Repo) ([]gh.Runner, error) {
	...
	return []gh.Runner{{ID: 222, Name: "runnerkit-" + repo.Owner + "-" + repo.Name + "-ephemeral-fake1", OS: "linux", Status: "online", Labels: []string{"self-hosted", "runnerkit", labels.RepoScopedLabel(repo), "linux", "x64", "ephemeral"}}}, nil
}
...
func TestUpEphemeralBYOEndToEndApplyEphemeralAndPersistsState(t *testing.T) {
	...
	for _, want := range []string{
		...
		if !strings.Contains(stdout, want) {
			t.Fatalf("ephemeral BYO completion missing %q:\n%s", want, stdout)
		}
	}
	ids := []string{}
